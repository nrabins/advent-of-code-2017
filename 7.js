...
class Node {
  constructor(line) {
    ...
    const weightMatches = weightRegex.exec(chunks[1]);
    this.weight = parseInt(weightMatches[1]);

    const childrenStr = line.split(" -> ")[1];
    if (childrenStr != undefined) {
      this.childrenStrs = childrenStr.split(", ");
    }
  }
}

const lines = input.split('\n');
var parentName = findParent(lines[0], lines);

// pick a node
// see if that node exists as a child of any other node
// if not, that's the parent node!
// if so, start over with the parent of that sub-tree
function findParent(currNode, nodes) {
  const name = currNode.split(" (")[0];
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    const childrenStr = node.split(" -> ")[1];
    if (childrenStr != undefined) {
      const children = childrenStr.split(", ");
      const parentIndex = children.findIndex(x => x == name);
      if (parentIndex != -1) {
        return findParent(nodes[i], nodes);
      }
    }
  }
  return name;
...
console.log('the parent is ' + parentName);
...
function constructTree(name, nodes) {
  let node = findNode(name, nodes);

  if (node.childrenStrs) {
    node.children = node.childrenStrs.map(childStr => constructTree(childStr, nodes));
    delete node.childrenStrs;
  }
  return node;
}
...
function calculateWeight(node) {
  ...
  if (node.children) {
    node.children.forEach(child => {
      totalWeight += calculateWeight(child);
    });
  }
...
function findUnbalancedParent(node) {
  if (node.children && node.children.length > 0) {
    
    const childValues = node.children.reduce( (tally, child) => {
      tally[child.totalWeight] = (tally[child.totalWeight] || 0) + 1;
      return tally;
    }, {})

    const outlierWeight = Object.getOwnPropertyNames(childValues).find(key => {
      return childValues[key] == 1;
    })

    if (outlierWeight) {
      console.log(node.name + " is unbalanced!");
      console.log(node.children.map(child => child.name + ": " + child.totalWeight).join(", "));
      
      const outlierNode = node.children.find(child => child.totalWeight == outlierWeight);
      const modeNode = node.children.find(child => child.totalWeight != outlierWeight);
      console.log(outlierNode.name + " should weigh " + modeNode.totalWeight + " total.");
      
      const desiredWeight = outlierNode.weight - (outlierNode.totalWeight - modeNode.totalWeight);
      console.log("To achieve this, change its weight to " + desiredWeight + ".");
    }

    node.children.forEach(child => findUnbalancedParent(child));
  }
}

const parentNode = constructTree(parentName, nodes);

const totalWeight = calculateWeight(parentNode);

const unbalancedParent = findUnbalancedParent(parentNode);