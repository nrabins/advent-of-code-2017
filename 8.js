...
class Instruction {
    constructor(line) {
        ...
        this.alterRegister = chunks[0];
        
        let amount = parseInt(chunks[2]);
        if (chunks[1] == 'dec') {
            amount = -1 * amount;
        }
        this.amount = amount;

        this.compareRegister = chunks[4];
        this.compareOperator = chunks[5];
        this.compareValue = chunks[6];

        this.shouldDoOperation = () => {
            const registerValue = registers[this.compareRegister];
            switch (this.compareOperator) {
                case "<":   return registerValue < this.compareValue; break;
                case ">":   return registerValue > this.compareValue; break;
                case "<=":  return registerValue <= this.compareValue; break;
                case ">=":  return registerValue >= this.compareValue; break;
                case "==":  return registerValue == this.compareValue; break;
                case "!=":  return registerValue != this.compareValue; break;
                default:    console.log("unrecognized operation: " + this.compareOperator);
            }
        }

        this.run = () => {
            if (this.shouldDoOperation()) {
                registers[this.alterRegister] += this.amount;
                overallMax = Math.max(registers[this.alterRegister], overallMax);
            }
        }
...
instructions.forEach(instruction => {
    registers[instruction.alterRegister] = 0;
    registers[instruction.compareRegister] = 0;
});
...
const maxValue = Math.max(...Object.getOwnPropertyNames(registers).map(key => registers[key]));
